...
/** Pivots within this fraction of each other are the same level. */
const CLUSTER_TOLERANCE = 0.018;
...
function findPivots(bars: ChartPoint[]): Pivot[] {
  ...
  for (let i = PIVOT_WINDOW; i < bars.length - PIVOT_WINDOW; i++) {
    const win = bars.slice(i - PIVOT_WINDOW, i + PIVOT_WINDOW + 1);
    const b = bars[i];
...
/** Group pivots that sit within CLUSTER_TOLERANCE of one another. */
function clusterPivots(pivots: Pivot[]): Pivot[][] {
  ...
  for (const p of sorted.slice(1)) {
    ...
    if (Math.abs(p.price - ref) / ref <= CLUSTER_TOLERANCE) cur.push(p);
    else {
      groups.push(cur);
      cur = [p];
    }
...
export function deriveLevels(bars: ChartPoint[]): LevelAnalysis | null {
  if (bars.length < PIVOT_WINDOW * 4) return null;

  const price = bars[bars.length - 1].close;
  const pivots = findPivots(bars);

  const zones: LevelZone[] = clusterPivots(pivots)
