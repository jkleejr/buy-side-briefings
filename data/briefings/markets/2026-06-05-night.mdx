...
- **VIX ~20.9 est.** — trigger above 18 **BREACHED — ABOVE 20** (surged ~34% from ~15.60 June 4 close; rate-hike fear and SOX structural unwind drove the spike; not systemic credit stress; VIX term structure will reveal whether this is event risk [near-term spike, longer-dated calm] or structural [parallel upward shift])
- **30Y Yield 5.007%** — trigger above 5.05% **BREACHED — DAY-2 SEQUENCE VOIDED** (crossed the 5.00% framework threshold for the first time since the June 1 recovery; only 4bps from the 5.05% secondary trigger; must close below 5.00% on two consecutive sessions to re-establish any constructive rate narrative)
...
- **SPX 7,383.74 vs 7,460** — trigger below 7,460 **BREACHED — 76 POINTS BELOW FLOOR** (framework floor broken; next meaningful support at 200-day moving average; reclaiming 7,460 is the prerequisite for any HOLD upgrade, not a sufficient condition alone)
