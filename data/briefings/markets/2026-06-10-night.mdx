...
| | Level | Change | Note |
|---|---|---|---|
| S&P 500 | 7,266.99 | −1.62% | 193 POINTS BELOW 7,460 FLOOR — also 33 pts below morning brief's explicit 7,300 BEARISH trigger |
...
- **VIX ~19.87** — trigger above 18 **BREACHED — STABLE** (flat from June 9's est. 19.80; not compressing despite two consecutive sell sessions; the lack of vol spike on a −1.62% SPX day suggests participants are not panicking — but not recovering either; 22+ = capitulation signal; VIX close below 17 remains the first constructive vol-regime signal and is further away than this morning)
- **30Y Yield ~4.999%** — trigger above 5.00% **AT THRESHOLD — ONE PPI FROM BREACH** (flat for a second consecutive session at the precipice; May PPI Thursday at 8:30 AM is the decisive event; consensus is +0.7% m/m / +6.4% YoY — a print ≥0.8% m/m pushes 30Y to 5.10-5.20% immediately; core CPI 2.9% is the pipeline counterargument)
- **DXY 100.01** — trigger above 100 **RE-BREACHED** (second episode above 100 after a one-session break below; hot headline CPI + Iran escalation reversed the dollar's downtrend; whether DXY holds above 100 depends on PPI Thursday and Iran overnight)
- **SPX 7,266.99 vs 7,460** — trigger below 7,460 **BREACHED — 193 POINTS BELOW FLOOR** (worst reading since June 5; morning brief's supplemental BEARISH trigger of 7,300 also breached at close; next support 7,200-7,250; a close below 7,200 activates BEARISH verdict)
- **Brent $93.10** — trigger above $97 **$3.90 BELOW TRIGGER — APPROACHING** (active US-Iran military exchange in Hormuz region; Hormuz mine threat uncleared; a tanker incident is a non-linear spike event — this buffer can close in one session)
