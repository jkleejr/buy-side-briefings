...
| | Level | Change | Note |
|---|---|---|---|
| S&P 500 | 7,420.10 | **-1.21%** | June 17 close; 40 pts below 7,460 regime floor; ES futures 7,556.50 pre-market implies gap-up above floor at open close must confirm |
...
- **DXY 99.65** — CLEARED but NARROW (0.35 pts buffer). DXY is condition 3 for BUY. The Iran deal is the structural dollar-weakening catalyst — oil down 10% in six sessions, sanctions on Iranian energy being lifted, and Hormuz reopening removes the geopolitical risk premium from the dollar. Watch for DXY to break below 99.00 today; if it does AND the other two conditions are met, HOLD upgrades to BUY.

- **SPX 7,420.10** — BREACHED — 40 pts below regime floor. ES at 7,556.50 pre-market implies SPX opens above 7,460. A CLOSE above 7,460 is condition 2 for BUY. Note: a gap-open above 7,460 does not satisfy the condition — it must be sustained through the session and confirmed at the 4 PM ET close.
...
- **VIX 18.44** — trigger above 18 **BREACHED** (June 17 close; first close above 18 this cycle; NVDA add deactivated; VIX < 18 close is condition 1 for HOLDBUY upgrade; Iran-relief gap-up should bring VIX lower today but 8:30 AM ET data and 30Y dynamics could reverse that; watch intraday VIX alongside SPX — if VIX is rising while SPX is rising, that's a warning of hidden selling pressure)

- **30Y ~4.94% est.** — trigger above 5.00% **CRITICALLY NARROW — 6 bps buffer** (most dangerous regime indicator today; equity rally is paradoxically risk-on which means bond selling = higher yields; Warsh's dot-plot review removes the forward-guidance anchor on the long end; if bond traders sell duration into the Iran gap-up rally, the 30Y could test 5.00% before today's close; a 30Y close above 5.00% converts HOLD to STEP_ASIDE immediately regardless of SPX level)
...
- **SPX 7,420.10** — trigger below 7,460 **BREACHED** (40 pts below floor at June 17 close; ES pre-market 7,556.50 implies SPX opens above 7,460; needs to CLOSE above 7,460 for condition 2; a gap-up that fades and closes below 7,460 is a distribution pattern confirming the June 16–17 double-top)
